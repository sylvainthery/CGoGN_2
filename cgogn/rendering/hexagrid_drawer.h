...
public:
	...
	FakeAttribute(const T& v):
		value_(v) {}
	template <typename C>
	T operator[](C) const { return value_;}
};
 ...
 * Typical usage:
 *
 *  std::unique_ptr<cgogn::rendering::HexaGridDrawer> volu_;	// can be shared between contexts
 *  std::unique_ptr<cgogn::rendering::HexaGridDrawer::Renderer> volu_rend_; // one by context,
 *
 * init:
...
public:
	...
	void update_edge(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VOL_UATTR& vol_I, const VOL_UATTR& vol_J, const VOL_UATTR& vol_K);

	template <typename MAP, typename VERTEX_ATTR, typename VOL_UATTR>
	void update_edge(const MAP& m, const VERTEX_ATTR& position, const VOL_UATTR& vol_I, const VOL_UATTR& vol_J, const VOL_UATTR& vol_K)
	{
		update_edge(m, AllCellsFilter(), position, vol_I, vol_J, vol_K);
	}

	template <typename MAP, typename MASK, typename VERTEX_ATTR , typename VOL_UI_ATTR>
	void update_face(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VERTEX_ATTR& color, const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K);

	template <typename MAP, typename VERTEX_ATTR, typename VOL_UI_ATTR>
	inline void update_face(const MAP& m, const VERTEX_ATTR& position, const VERTEX_ATTR& color,const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K)
	{
		update_face(m, AllCellsFilter(), position, vol_I,vol_J,vol_K);
	}

	template <typename MAP, typename MASK, typename VERTEX_ATTR, typename VOL_ATTR, typename VOL_UI_ATTR>
	void update_face(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VOL_ATTR& color, const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K);

	template <typename MAP, typename VERTEX_ATTR, typename VOL_ATTR, typename VOL_UI_ATTR>
	inline void update_face(const MAP& m, const VERTEX_ATTR& position, const VOL_ATTR& color,const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K)
	{
...
template <typename MAP, typename MASK, typename VERTEX_ATTR, typename VOL_UATTR>
void HexaGridDrawer::update_edge(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VOL_UATTR& vol_I, const VOL_UATTR& vol_J, const VOL_UATTR& vol_K)
{
	...
	m.foreach_cell([&] (Volume v)
	{
		...
		m.foreach_incident_edge(v, [&] (Edge e)
		{
			...
			out_pos.push_back({float32(vol_I[v]), float32(vol_J[v]), float32(vol_K[v])});
	...
	template <typename MAP, typename MASK, typename VERTEX_ATTR , typename VOL_UI_ATTR>
	void HexaGridDrawer::update_face(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VERTEX_ATTR& color, const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K)
	{
		static_assert(is_orbit_of<VERTEX_ATTR, MAP::Vertex::ORBIT>::value,"position must be a vertex attribute");
		...
		m.foreach_cell([&] (Volume v)
		{
			...
			m.foreach_incident_face(v, [&] (Face f)
			{
				if (m.has_codegree(f, 3))
				{
					...
					out_color.push_back({float32(vol_I[v]), float32(vol_J[v]), float32(vol_K[v])});
	...
	template <typename MAP, typename MASK, typename VERTEX_ATTR, typename VOLUME_ATTR, typename VOL_UI_ATTR>
	void HexaGridDrawer::update_face(const MAP& m, const MASK& mask, const VERTEX_ATTR& position, const VOLUME_ATTR& color, const VOL_UI_ATTR& vol_I, const VOL_UI_ATTR& vol_J, const VOL_UI_ATTR& vol_K)
	{
		static_assert(is_orbit_of<VERTEX_ATTR, MAP::Vertex::ORBIT>::value,"position must be a vertex attribute");
